...
public class Teleport implements net.ess3.api.ITeleport {
    private final com.earth2me.essentials.IUser teleportOwner;
    private final com.earth2me.essentials.IEssentials ess;
    private TimedTeleport timedTeleport;

    public Teleport(com.earth2me.essentials.IUser user, IEssentials ess) {
    ...
    private void warnUser(final com.earth2me.essentials.IUser user, final double delay) {
    ...
    protected void now(com.earth2me.essentials.IUser teleportee, ITarget target, TeleportCause cause) throws Exception {
        cancel(false);
    ...
    public void teleportPlayer(com.earth2me.essentials.IUser teleportee, Location loc, Trade chargeFor, TeleportCause cause) throws Exception {
        teleport(teleportee, new LocationTarget(loc), chargeFor, cause);
    }

    public void teleportPlayer(net.ess3.api.IUser teleportee, Location loc, Trade chargeFor, TeleportCause cause) throws Exception {
        teleport(teleportee, new LocationTarget(loc), chargeFor, cause);
    }

    //This is used on /tphere
    public void teleportPlayer(com.earth2me.essentials.IUser teleportee, Player entity, Trade chargeFor, TeleportCause cause) throws Exception {
        ITarget target = new PlayerTarget(entity);
    ...
    public void teleportPlayer(net.ess3.api.IUser teleportee, Player entity, Trade chargeFor, TeleportCause cause) throws Exception {
        ITarget target = new PlayerTarget(entity);
        teleport(teleportee, target, chargeFor, cause);
        teleportee.sendMessage(tl("teleporting", target.getLocation().getWorld().getName(), target.getLocation().getBlockX(), target.getLocation().getBlockY(), target.getLocation().getBlockZ()));
        teleportOwner.sendMessage(tl("teleporting", target.getLocation().getWorld().getName(), target.getLocation().getBlockX(), target.getLocation().getBlockY(), target.getLocation().getBlockZ()));
    ...
    private void teleport(com.earth2me.essentials.IUser teleportee, ITarget target, Trade chargeFor, TeleportCause cause) throws Exception {
    ...
    protected void respawnNow(com.earth2me.essentials.IUser teleportee, TeleportCause cause) throws Exception {
    ...
    public void warp(com.earth2me.essentials.IUser teleportee, String warp, Trade chargeFor, TeleportCause cause) throws Exception {
    ...
    public void warp(net.ess3.api.IUser teleportee, String warp, Trade chargeFor, TeleportCause cause) throws Exception {
        Location loc = ess.getWarps().getWarp(warp);
        teleportee.sendMessage(tl("warpingTo", warp, loc.getWorld().getName(), loc.getBlockX(), loc.getBlockY(), loc.getBlockZ()));
        if (!teleportee.equals(teleportOwner)) {
            teleportOwner.sendMessage(tl("warpingTo", warp, loc.getWorld().getName(), loc.getBlockX(), loc.getBlockY(), loc.getBlockZ()));
