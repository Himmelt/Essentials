    ...
    private class ConstructBukkitScalar extends ConstructScalar {
        ...
        public Object construct(final Node node) {
            if (node.getType().equals(Material.class)) {
                final String val = constructScalar((ScalarNode) node);
                Material mat;
        ...
        public Object construct(final Node node) {
            if (node.getType().equals(Location.class)) {
                ...
                for (NodeTuple nodeTuple : mnode.getValue()) {
                    final String key = constructScalar((ScalarNode) nodeTuple.getKeyNode());
                    final ScalarNode snode = (ScalarNode) nodeTuple.getValueNode();
                    if (key.equalsIgnoreCase("world")) {
                        worldName = constructScalar(snode);
                    }
                    if (key.equalsIgnoreCase("x")) {
                        x = Double.parseDouble(constructScalar(snode));
                    }
                    if (key.equalsIgnoreCase("y")) {
                        y = Double.parseDouble(constructScalar(snode));
                    }
                    if (key.equalsIgnoreCase("z")) {
                        z = Double.parseDouble(constructScalar(snode));
                    }
                    if (key.equalsIgnoreCase("yaw")) {
                        yaw = Float.parseFloat(constructScalar(snode));
                    }
                    if (key.equalsIgnoreCase("pitch")) {
                        pitch = Float.parseFloat(constructScalar(snode));
                    }
                }
